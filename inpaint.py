...
import os, sys
sys.path.append(os.getcwd() + '/pytorch-siggraph2017-inpainting')
from utils import *
from poissonblending import prepare_mask, blend
from glcic import glcic

parser = argparse.ArgumentParser()
parser.add_argument('--input', default='pytorch-siggraph2017-inpainting/example.png', help='Input image')
parser.add_argument('--mask', default='pytorch-siggraph2017-inpainting/example_mask.png', help='Mask image')
parser.add_argument('--model_path', default='completionnet_places2.pt', help='Trained model')
...
model = glcic(in_ch=4, out_ch=3, ch=64)
model.load_state_dict(torch.load(opt.model_path)['model'])
model.eval()
datamean = torch.Tensor([0.4560,  0.4472,  0.4155])
